...
def validate_project(project):
    if not isinstance(project, dict):
        logging.error(
            f"Invalid project format: {project}. Each project should be a dictionary."
        )
        return False
    ...
    for key in required_keys:
        if key not in project:
            ...
            return False
        if key == "port" and (
            not isinstance(project[key], int)
            or project[key] <= 0
            or project[key] > 65535
        ):
            logging.error(
                f"Invalid port '{project[key]}' for project: {project}. "
...
def create_nginx_config(project: dict, templates: dict):
    ...
    if os.path.exists(config_path) or os.path.exists(symlink_path):
        logging.info(
            f"Configuration for {project['name']} already exists. Skipping creation."
        )
        return False
